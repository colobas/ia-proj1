...
class HCBStateRepresentation(StateRepresentation):
	...
	def setup(self): #definir operacoes possiveis deste nó
		if type(self.CTS_pos) == Stack and self.cask_on_CTS != None and self.prev_operation != "load": #undoing previous operation not allowed
			if self.CTS_pos.space_left >= self.cask_on_CTS.length: # if cask fits in stack
				fcost = self.fcost + 1 + hcb.casks[cask_on_CTS].weigh
				gcost = 0
			...
			if self.hcb.nodes[self.CTS_pos].space_left != self.hcb.nodes[self.CTS_pos].size: # if stack isn't empty
				fcost = self.fcost + 1 + hcb.casks[cask_on_CTS].weight
				gcost = 0
		...
		for neighbour in self.hcb.nodes[self.CTS_pos].neighbours:
			if ((self.parent == None) or not (neighbour == self.parent.CTS_pos) and self.prev_operation == "move"): #moving back not allowed
				fcost = self.fcost + self.hcb.nodes[self.CTS_pos].neighbours[neighbour]
				gcost = 0
				def move_to_neighbour():
					return self.move(neighbour)
				self.operations["move"] = {
	...
	def move(self, to):
		...
		child = HCBStateRepresentation(self, self.hcb, next_cost, 0, next_stacks, next_CTS_pos, self.cask_on_CTS, "move")
		return child
